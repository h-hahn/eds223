...
```{r}
cpad_super <- st_read("../data/discussion_data/CPAD_2023a_SuperUnits.shp", quiet = TRUE) %>%
  sf::st_transform("EPSG:4326") %>%
  janitor::clean_names() %>%
  mutate(ID = row_number())

ghm <- rast("../data/discussion_data/gHM_masked.tif") %>%
  project("EPSG:4326")
```
...
ggplot() +
  ...
  theme_minimal() +
  scale_fill_viridis_d() +
  labs(x = "", y = "", fill = "Global Human Modification")
```
...
summary_boxplot = function(polygon, raster, my_layer, my_label) {
  ...
  zonal_layer <<- zonal(raster, id_rast, fun = "mean", na.rm = TRUE)
    # <<- creates global variable instead of temporary local variable
...
# when function works remove <<- and put <- so we don't store too many variables in global environment
# use <<- to see where things are/aren't working
```
