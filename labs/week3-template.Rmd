...
# filter nz height just in Canterbury
# figuring out by intersections
# keeping any geometries in nz_heights that intersect with Canterbury data set
...
```{r include=TRUE}
# disjoint
# find geometries that are disjoing with canterbury
...
```{r include=TRUE}
# st_intersects and st_intersection both find intersections but give different things
...
```{r include=TRUE}
nz_height %>% 
  st_filter(y = canterbury, .predicate = st_intersects) # .predicate telling it how to filter

# what does st_intersection do?
# gives 70 rows, 9 variables
# gives attribute info from other dataset
...
```{r include=TRUE}
# forces centroid to sit on region we're interested in
